...
class ResearchPlannerAgent:
    ...
    def execute(self, requirements: Dict[str, Any]) -> Dict[str, Any]:
        ...
        messages = [
            SystemMessage(content=self.SYSTEM_PROMPT),
            HumanMessage(content=f"Project requirements: {json.dumps(requirements, indent=2)}")
        ]
...
class DocumentationResearcherAgent:
    ...
    def execute(self, implementation_plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        messages = [
            SystemMessage(content=self.SYSTEM_PROMPT),
            HumanMessage(content=f"Tech stack: {json.dumps(implementation_plan.get('tech_stack_final', []))}")
        ]
