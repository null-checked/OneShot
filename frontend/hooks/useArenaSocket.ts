...
export function useArenaSocket(options: UseArenaSocketOptions = {}): ArenaSocketReturn {
    const { url = "ws://localhost:8000/ws/arena", autoReconnect = true, reconnectInterval = 3000 } = options;
        ...
        try {
            ...
            switch (wsEvent.type) {
                case "status": {
                    ...
                    setArenaState((prev: ArenaState) => ({ ...prev, status: data.stage }));
                    break;
                }

                case "problem_parsed": {
                    const data = wsEvent.data as ArenaState["problem"];
                    setArenaState((prev: ArenaState) => ({ ...prev, problem: data }));
                    break;
                }

                case "architect_start": {
                    setArenaState((prev: ArenaState) => ({
                        ...prev,
                ...
                case "agent_task_assigned": {
                    ...
                    setArenaState((prev: ArenaState) => ({
                        ...prev,
                        agents: prev.agents.map((agent: ArenaState["agents"][0]) => (agent.personality.name === data.agent ? { ...agent, task: data.task, status: "idle" as const } : agent)),
                    }));
                    break;
                }
                ...
                case "agent_start": {
                    ...
                    setArenaState((prev: ArenaState) => ({
                        ...prev,
                        agents: prev.agents.map((agent: ArenaState["agents"][0]) => (agent.personality.name === data.agent ? { ...agent, status: "coding" as const } : agent)),
                    }));
                    break;
                }
                ...
                case "agent_complete": {
                    ...
                    setArenaState((prev: ArenaState) => ({
                        ...prev,
                        agents: prev.agents.map((agent: ArenaState["agents"][0]) =>
                ...
                case "test_start": {
                    ...
                    setArenaState((prev: ArenaState) => ({
                        ...prev,
                        agents: prev.agents.map((agent: ArenaState["agents"][0]) => (agent.personality.name === data.agent ? { ...agent, status: "testing" as const } : agent)),
                    }));
                    break;
                }
                ...
                case "test_results": {
                    ...
                    setArenaState((prev: ArenaState) => ({
                        ...prev,
                        agents: prev.agents.map((agent: ArenaState["agents"][0]) => (agent.personality.name === data?.agent_name ? { ...agent, testResults: data } : agent)),
                    }));
                    break;
                }
                ...
                case "scores": {
                    ...
                    setArenaState((prev: ArenaState) => ({
                        ...
                        agents: prev.agents.map((agent: ArenaState["agents"][0]) => {
                            ...
                            return score ? { ...agent, status: "complete" as const, score } : { ...agent, status: "complete" as const };
                        }),
                    }));
                    break;
                }
                ...
                case "error": {
                    ...
                    setArenaState((prev: ArenaState) => ({ ...prev, status: "error", error: data.message }));
                    break;
                }
